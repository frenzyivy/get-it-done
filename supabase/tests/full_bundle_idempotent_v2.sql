...
-- DROP first because CREATE OR REPLACE VIEW cannot rename or reorder existing
-- columns. Live DBs may hold an older shape (e.g. with task_title in a slot
-- the new definition uses for a different column). CASCADE catches any
-- downstream views/objects that reference this one.
DROP VIEW IF EXISTS v_planned_vs_actual CASCADE;
...
-- DROP first to escape the same CREATE OR REPLACE column-rename limitation.
DROP VIEW IF EXISTS v_unplanned_sessions CASCADE;
...
-- Idempotent via DROP + CREATE OR REPLACE. (Plain CREATE OR REPLACE alone
-- can't rename existing columns; this is the safe form.)

DROP VIEW IF EXISTS v_task_status CASCADE;
